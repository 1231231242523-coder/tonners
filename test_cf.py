...
async def main():
    async with async_playwright() as p:
        ...
        xhr_result = await page.evaluate("""
            () => {
                return new Promise((resolve, reject) => {
                    ...
                    xhr.open('GET', window.location.href + '?page=2&from-xhr', true);
                    xhr.setRequestHeader('X-Requested-With', 'XMLHttpRequest');
                    xhr.setRequestHeader('Accept', 'application/json, text/javascript, */*; q=0.01');
                    
                    xhr.onload = () => {
                        if (xhr.status === 200) {
                            resolve({ status: xhr.status, content: xhr.responseText.substring(0, 500) });
                        } else {
        ...
        for cookie in cookies:
            if 'PHPSESSID' in cookie.get('name', '') or 'PrestaShop' in cookie.get('name', ''):
